...
app.get('/character',(req,res)=> {
   ...
   * SELECT QUERY
   */
app.get('/character/type/:id',(req,res)=> {
    var charT_sql = "SELECT Character.id,Character.name, Type.type_name, Group.group_name, Character.gender, City.city_name FROM `Character` LEFT JOIN `Type` ON Character.type_id = Type.id LEFT JOIN `Group` ON Character.group_id = Group.id LEFT JOIN `City` ON Character.city_id = City.id WHERE type_id =?";   
    pool.query(charT_sql,[req.params.id],(err,rows,result,fields)=>{
        if(err)
        {
            res.json(err);
            console.log(err);
   ...
   * SELECT QUERY
   */
app.get('/character/group/:id',(req,res)=> {
    ...
    pool.query(charG_sql,[req.params.id],(err,rows,result,fields)=>{
        if(err)
        {
            res.json(err);
            console.log(err);
